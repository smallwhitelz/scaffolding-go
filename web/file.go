...
func (d *FileDownloader) Handle() HandleFunc {
	return func(ctx *Context) {
		...
		if err != nil {
			...
			ctx.RespData = []byte("找不到目标文件" + err.Error())
			return
		}
...
// StaticResourceHandler 两个层面控制缓存
// 1. 大文件不缓存
// 2. 控制了文件缓存的数量
...
func (s *StaticResourceHandler) Handle(ctx *Context) {
	...
	if data, ok := s.cache.Get(file); ok {
		fmt.Println(1111)
		header.Set("Content-Type", s.extensionContentTypeMap[ext[1:]])
